...
#### Correcciones de seguridad (audit interno)

- `passwords:list` verifica que el solicitante sea la ventana principal y no un webview — los iframes del juego no pueden listar contraseñas guardadas.
- WeakSet guard en `handleDownloads` evita que se registren listeners duplicados al cerrar y reabrir cuentas varias veces en la misma sesión.
- URLs de bookmarks importados se sanitizan: se rechazan esquemas `javascript:`, `file://` y `data:`.
- Escritura del archivo de datos usa `writeFileSync` + `renameSync` atómico — si el proceso se interrumpe a mitad, el archivo viejo queda intacto.

---

#### Tests añadidos

- **28 tests unitarios** nuevos: 18 para helpers de market (parsing de frames, filtros de precio, lógica de feed), 10 para seguridad (sanitización de URLs de bookmarks, patrón WeakSet de deduplicación de listeners).
- **Test e2e** de regresión: cierre y reapertura de cuentas no corrompe el estado del sidebar (cubre bug B-01).
